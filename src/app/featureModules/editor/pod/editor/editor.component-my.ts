...
export class EditorComponent implements AfterViewInit, OnInit {
  ...
  processSectionArrayData(data, execType) {
    ...
    for (let i = 0; i < data.length; i++) {
      if (execType == 'set') {
        ...
        let isbelongTo = elem.children[0].children[0];
        let attr = isbelongTo.getAttribute('isbelongto') == "" ? isbelongTo.children[0].getAttribute('isbelongto') == "" ?
    ...
    if (!found) {
      for (let i = 0; i < jsonData.length; i++) {
        ...
        if (obj == "attributes") {
          ...
          this.updateJsonData(jsonData[i][obj], data, found);
        }
      }
    }
  }
  ...
  async checkComponent(htmlJson, customJson, comp_Data, index) {
    ...
    for (let i = 0; i < htmlJson.length; i++) {
      ...
      if (obj == "attributes") {
        ...
        if(htmlJson[i].componentData[0].type.toLowerCase() == 'mjx-container'){
          ...
          for (let k = 0; k < customJson.length; k++) { // custom component array
            ...
            if (startIndex != -1 && key1.substring(startIndex) == key2) {
              ...
              if(htmlJson[i].attributes[0].is_component == "false"){
                await this.setKeyAttribute(comp_Data, removeElements, 2, key2, index);
                k = customJson.length;
              }
              // }
            }
        ...
        this.checkComponent(htmlJson[i][obj], this.jsonArray, comp_Data, i);
      }
    }
  }
      ...
      if (obj != "attributes" && obj != "text" && obj != "img") {
        if (obj == removeElements[index]) {
          ...
          if (htmlJson[i][temp] != undefined && (temp != 'p' || (htmlJson[i][temp].length > 1 && htmlIndex == i))) { //i == htmlIndex
            htmlJson[i][temp][0].attributes[0]['key'] = cus_key;
            htmlJson[i][temp][0].attributes[0]['is_component'] = "true";
            i = htmlJson.length;
          }
        }
      }
    ...
    for (let i = 0; i < data.length; i++) {
      if (this.selectedJson.length < 1) {
        ...
        if (obj == "attributes") {
          ...
          else if((data[i].componentData[0].type.toLowerCase() == 'mjx-container' || data[i].componentData[0].type.toLowerCase() == 'table') && (tagName == 'mjx-container' || tagName == 'table')){
          ...
          for (let x in data) {
            if (data[x].fileName == 'editorHeadJSON') {
              ...
              for (let i = 0; i < this.html_body_json.length; i++) {
                ...
                if (this.html_body_json[i][obj].length > 1) {
                  await this.checkComponent(this.html_body_json[i][obj], this.jsonArray, '', '');
                }
              }
