...
export class editorDomService {
  ...
  appendIframeHeadContentNew = async function (json, contentDoc) {
    this.pub_sub.minorColumn = [];
    try {
      contentDoc.head.innerHTML = '';
      if (json != "") {
        ...
        for (let key in data) {
          ...
          // let checkCSS = data[key][Object.keys(data[key])[0]][0]['attributes'][0];
          let checkCSS = data[key]["attributes"][0];

          if (elem_type == 'link' && checkCSS.href && (checkCSS.href).indexOf('iframe.css') > -1) {
            checkCSS.href = "../../../pod_assets/uploads/iframe.css";
          }

          let resdata = await this.createElementAttribute(elem, checkCSS);
  ...
  appendIframePageWiseContentNew = async function (page_model, contentDoc, pageNo, testDoc) {
    ...
    try {
      ...
      for (let keyx in page_model.json) {
        ...
        let nextElement = parseInt(keyx) + 1;
        let prevElement = parseInt(keyx) - 1;
        let elem_test = await this.createElementNew('', page_model.json[keyx], this.page_sec, page_model.json, page_model.json[prevElement], page_model.json[nextElement], keyx, true);
        if (!this.condition_return && (this.pub_sub.elemType == 'pagebreakInside' || this.pub_sub.elemType == 'default')) {
          let key_1 = Object.keys(page_model.json[JSON.parse(keyx)])
          if (!page_model.json[keyx][key_1[0]][0].hasOwnProperty('attributes'))
            break;
      ...
      for (let keyx in page_model.json) {
        // this.pub_sub.resetId = this.pub_sub.resetId == ''?this.pub_sub.startFromUniqID:this.pub_sub.resetId;
        //console.log(page_model.json[keyx])
        await this.pdfEditorElementStruture.resetJSONNew(page_model.json[keyx], this.isSave, this.sectionCount, this.pub_sub);
      }
      //}
      if (this.pub_sub.minorColumn.length >= 1)
        await this.minorColumnNew(this.pub_sub.minorColumn, contentDoc, true)
      // let copy = this.sec_temp.innerHTML;
      // this.iframeBody.innerHTML='';
      // this.iframeBody.innerHTML = copy;
      // let section_elements = this.iframeBody.querySelectorAll( 'body > *' );
  ...
  minorColumnNew = async function (array, contentDoc, checkMajorIdExist) {
    this.minor_column = true;
    if (checkMajorIdExist) {
      for (var i = 0; i < array.length; i++) {
        if ((contentDoc.body.getElementsByClassName(array[i].id).length == 0) && array[i].status == true) {
      ...
      for (var i = 0; i < rev_array.length; i++) {
        if (contentDoc.body.getElementsByClassName(rev_array[i].id).length > 0) {
          ...
          let sec_temp = this.renderer.createElement('section')
          sec_temp.setAttribute('id', 'removable')
          await this.createElementNew(sec_temp, rev_array[i].data, '');
          let elem: any = rev_array[i].parentId == 'body' ? contentDoc.body : contentDoc.body.querySelectorAll("[uniqid='" + rev_array[i].parentId + "']")[0];
          let appendAtIndex: any = '';
          if (rev_array[i].afterElemId == '') {
            appendAtIndex = 0;
  ...
  /** function to create element */
  createElementNew = async function (parent, json, contentDocBody, cdata, prev, next, index, count, state) {
    if (!this.isDestroy) {
      // console.log(json);
      let elem_type: any = '';
      let element: any = '';
      ...
      if (att.hasOwnProperty('isbelongTo') && !this.startIndex && !this.noBreak) {
        if (att.isbelongTo != '')
          return parent ? parent : contentDocBody;
      }
      if (att['class'] != undefined && !this.noBreak) {
        if (att['class'].includes('minor-column')) {
          if (!this.pub_sub.minorColumn.some((x) => { return x.id == att['minor_id'] })) {
            this.pub_sub.minorColumn.push({
              'id': att['minor_id'],
              'data': json,
              'parentId': (parent != '' || parent != undefined) ? (parent.classList.contains('mhhe-pod--glos-par')) ? parent.parentNode.getAttribute('uniqid') : parent.getAttribute('uniqid') : 'body',
      ...
      att.isbelongTo = 'page_' + this.sectionCount;
      ...
      // data = json[elem_type];
      try {
        for(let k = 0; k < data.fields.length; k++){
          if (!this.breakLoop && element && element.nodeName && element.nodeName !="") {
            // if (Object.keys(data[key])[0].toLowerCase() == 'attributes' || Object.keys(data).includes('attributes')) {
              await this.createElementAttribute(element, data.fields[k]['attributes'][0]);
              if (data.fields[k]["attributes"][0].uniqid == this.pub_sub.startFromUniqID && !this.isSave) {
                await this.setStartValue(element, data.fields[k], i)
                // this.startIndex = true;
              }
              // if(!element.classList.contains('mhhe-pod--minor-float') || !element.classList.contains('mhhe-pod--glos-entry')){
              if (element.nodeName.toLowerCase() == 'img') {
            ...
            if(data.fields[k].name == 'text'){  //if (Object.keys(data[key])[0].toLowerCase() == 'text') {
             
              if (data.fields[k]["attributes"][0].uniqid == this.pub_sub.startFromUniqID && element.nodeName.toLowerCase() != "li") {
               
                  // await this.setStartValue(element, data, key)
                  await this.setStartValue(element, data.fields[k], k)
              
              } else {
                await this.appendTextNode(element, data.fields[k].fields, this.pub_sub.paraStartFrom);
                //await this.timeout(50)
              }
              if (this.pub_sub.minorColumn.length >= 1 && !this.minor_column) {
                for (var i = 0; i < this.pub_sub.minorColumn.length; i++) {
                  if ((this.iframe_body.body.getElementsByClassName(this.pub_sub.minorColumn[i].id).length > 0) && this.pub_sub.minorColumn[i].status != true) {
                    await this.minorColumnNew(this.pub_sub.minorColumn, this.iframe_body, false)
                    this.minor_column = false;
                    this.noBreak = false;
                    this.breakLoop = false;
                  }
              ...
              if (data.fields[k]["attributes"][0].uniqid == this.pub_sub.startFromUniqID) {
                // startInsert = true;
                this.startIndex = true;
              }
              await this.timeout(25)
              ...
              if (element.getAttribute("rotate-section") && cdata[count + 1] && cdata[count + 1]["attributes"][0].class != "empty mhhe-pod--header-previous") {
                // console.log(cdata,count)
                if (cdata[count + 1]) {
                  var nxtElemHeight = parseInt(cdata[count + 1]["attributes"][0].offsetHeight);
                  //console.log("nxt",nxtElemHeight)
                  balh = balh + nxtElemHeight;
                  //console.log("bal",balh)
                }
              ...
              await this.createElementNew(element, data.fields[k]['0'], '', data.fields[k], data.fields[k][pre], data.fields[k][ne], data.fields[k][ne + 1], ne, true);
  ...
  */
  ...
  elementNotRemoveCondition = function (data, key) {
    ...
    if ((this.compress_object && (this.pub_sub.elemType.split('_')[1] == 'dontRemove' && (data.hasOwnProperty(this.pub_sub.DOMelement.nodeName.toLowerCase()))))) {
      this.pub_sub.startFromUniqID = '';
    ...
    try {
      ...
      for (let key in json) {
        if (key == 'computedStyles')
          continue
        if (json[key] != '' && json[key] != undefined && key != "offsetHeight")
  ...
  mouseClickEvent = (event, listType) => {
    ...
    if (event.target.tagName !== 'BODY' && tableElementTagName !== 'table' && mathJaxClass.indexOf(event.target.classList[0]) === -1 && mathM.toUpperCase() !== 'M') {
      ...
      document.getElementById("two-column-btn").classList.remove("active");
      document.getElementById("three-column-btn").classList.remove("active");
    }
