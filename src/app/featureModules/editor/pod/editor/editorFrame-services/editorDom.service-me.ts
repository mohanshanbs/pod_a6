...
// }
              ...
              {	
                ...
                {	
                  this.cumulative_rowheight += element.offsetHeight;	
                  if(this.cumulative_rowheight > element.parentNode.parentNode.parentNode.offsetWidth)	
                  {	
                    this.pub_sub = this.pdfEditorElementStruture.resetObj(this.pub_sub);	
                    this.breakLoop = true;	
                  }	
              ...
              await this.timeout(25)
               ...
               if(element.parentNode.parentNode.parentElement && element.parentNode.parentNode.parentElement.getAttribute("text")){  
                ...
                if(this.th_arr && this.th_arr.length > 0 && (this.th_arr[0].childNodes[0].children.length > 1 || this.th_arr[0].childNodes[0].classList.contains("backgound_lightblue")) && element.nodeName.toLowerCase() == 'table' && this.pub_sub.tableObj && this.pub_sub.tableObj.objStartFrom > 1){                          
            ...
            if (this.pub_sub.elemType.split('_')[1] != 'dontRemove' && !this.condition_return) {
              switch (this.pub_sub.elemType.toLowerCase()) {
                ...
                case 'list':
                  ...
                  if (this.pub_sub.elemType == 'list' && data[JSON.parse(key)].hasOwnProperty('text')){
                  ...
                  if (this.pub_sub.elemType == 'table') {
                    ...
                    if (this.pub_sub.tableObj.firstThreeRows) {
                      ...
                      if (data[JSON.parse(key)].hasOwnProperty('table')) {
                        let temp_table = element.childNodes[Math.round((JSON.parse(key)) - 1)]; console.log(temp_table)
                        ...
                        // else if(temp_table.nodeName.toLowerCase() == 'table' && Array.from(temp_table.parentNode.childNodes).indexOf(temp_table) > 0 && temp_table.previousElementSibling.nodeName.toLowerCase() == 'ol'){
                        //     //temp_table = element;
                        ...
                        // }
                          ...
                          if (data[JSON.parse(key)].hasOwnProperty('table') && element.previousElementSibling && element.previousElementSibling.nodeName.toLowerCase() == "header" && element.parentNode && element.parentNode.nodeName.toLowerCase() == "aside" && Math.round(this.sec_temp.getBoundingClientRect().height) > 900) {
                            ...
                            this.pub_sub.isSibling = false;
                            this.pub_sub.elemType = this.pub_sub.elemType.split('_')[1] == 'dontRemove' ? this.pub_sub.elemType : 'pagebreakInside';
                          } else if(data[JSON.parse(key)].hasOwnProperty('table') && element.parentNode && element.parentNode.parentNode && (element.parentNode.parentNode.nodeName.toLowerCase() == "li"|| element.parentNode.parentNode.nodeName.toLowerCase() == "figure") && Math.round(this.sec_temp.getBoundingClientRect().height) > 900){
                            temp_table = element.parentNode.parentNode;
                            this.pub_sub.startFromUniqID = temp_table.getAttribute('uniqid');
                            this.pub_sub.elemType = temp_table.nodeName.toLowerCase();
                            this.pub_sub.DOMelement = temp_table;
