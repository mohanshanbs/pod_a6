...
export class editorDomService {
  ...
  appendIframePageWiseContent = async function(page_model, contentDoc, pageNo) {
    ...
    try {
      ...
      for (let keyx in page_model.json) { 
        ...
        let elem_test = await this.createElement('', page_model.json[keyx], this.page_sec, page_model.json, page_model.json[prevElement], page_model.json[nextElement],keyx);
  ...
  createElement = async function(parent, json, contentDocBody, cdata, prev, next, index, count) {
    if (!this.isDestroy) {
      // console.log(json);
      let elem_type: any = '';
      let element: any = '';
        ...
        for (let key in data) {
          ...
          if (!this.breakLoop) { 
            ...
            if (Object.keys(data[key])[0].toLowerCase() == 'attributes') {
              ...
              //    if(element.previousElementSibling.previousElementSibling.getAttribute("rotate-table")){
              //      let childElem = element.previousElementSibling.previousElementSibling.children[0].childNodes;
              //      let paraElem = [];
              //      for(var i=0;i<childElem.length;i++){
              //        if(childElem[i].nodeName.toLowerCase() == "p" && childElem[i].classList.contains("break_the_page")) {  
              //          paraElem.push(childElem[i]);  
              //        }
              //      }
              //      if(paraElem.length > 0) {
              //        this.pub_sub = this.pdfEditorElementStruture.resetObj(this.pub_sub);
              //        this.breakLoop = true;
              //      }
              ...
              // var nextElement = parseInt(key)+1;
              //        nextElement = data[nextElement];

              //  if(element.parentNode && element.parentNode.getAttribute("rotate-table")) {
              //    console.log(json,nextElement)
              //    //elem_type = Object.keys(json)[0];
              //    //await this.createElement(element, nx, '', nx)
              //  } 
              ...
              await this.timeout(25)
               ...
               /* code for white space fix -- start */
                /* var difference = function (a, b) { return Math.abs(a - b); }
                 ...
                 var attrkey = Object.keys(next)[0];
                 var firstIteration = true;
                
                
                 if(balh < this.viewerheight){
                    if(next[attrkey][0]["attributes"][0].class == "break_the_page" && cdata[count+1] != undefined || cdata[count+1] != null || cdata[count] != undefined || cdata[count] != null) {
                       ...
                       if(cdata[count] == undefined || cdata[count] == null || Object.keys(cdata[count])[0] == "section" || Object.keys(cdata[count])[0] == "div"){
                         break;
                       }
                       next = cdata[count];
                       this.createElement(element.parentNode, next, '', cdata,prev,next,key,count);                   
                       attrkey = Object.keys(next)[0];
                       ...
                       var filteredObj = cdata.find(function(item, i){
                         ...
                         if(itemkey != "attributes") {
                           if(item[itemkey][0]["attributes"][0].id == next[attrkey][0]["attributes"][0].id){
                         ...
                         var attributeHeight = parseInt(next[attrkey][0]["attributes"][0].offsetHeight);
               ...
               /* code for white space fix -- end */
                ...
                await this.createElement(element, data[key], '');//, data, data[pre], data[ne],data[ne + 1],ne);
                      ...
                      if (data[JSON.parse(key)].hasOwnProperty('table')) {
                        ...
                        if (temp_table.nodeName.toLowerCase() == 'table' && Array.from(temp_table.parentNode.childNodes).indexOf(temp_table) > 0 && temp_table.previousElementSibling.nodeName.toLowerCase() == 'figcaption') {
                          ...
                          if (data[JSON.parse(key)].hasOwnProperty('table') && element.previousElementSibling && element.previousElementSibling.nodeName.toLowerCase() == "header" && element.parentNode && element.parentNode.nodeName.toLowerCase() == "aside" && Math.round(this.sec_temp.getBoundingClientRect().height) > 900) {
                            ...
                            this.pub_sub.elemType = this.pub_sub.elemType.split('_')[1] == 'dontRemove' ? this.pub_sub.elemType : 'pagebreakInside';
                          } else if(data[JSON.parse(key)].hasOwnProperty('table') && element.parentNode && element.parentNode.previousElementSibling.nodeName.toLowerCase() == "header" && Math.round(this.sec_temp.getBoundingClientRect().height) > 900) {
                             //console.log(element)
