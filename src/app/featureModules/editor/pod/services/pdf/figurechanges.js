figureChangesClass = function(projectlist,two_col,chapter_number){ 
...
figureChangesClass.prototype.addBgclrFig= function(){
	...
	for(var bf=0;bf<bf_figuretags.length;bf++){ 		
		if(bf_figuretags[bf].className.indexOf('figfloatright')<=-1){
			...
			// 	// f_img[0].className='';
			// 	var img_classlist=f_img[0].className.split(' ');
			// 	if(img_classlist.length > 0){
			// 		for (var c = 0; c < img_classlist.length; c++) {
			// 			if(img_classlist[c].indexOf('mhe--pod') <= -1 && img_classlist[c]!='' && !f_img[0].closest('table') && !f_img[0].closest('math')){
			// 				bf_figuretags[bf].classList.add(f_img[0].className);
			// 				f_img[0].classList.remove(img_classlist[c]);
			// 			}					
			// 		}
			// 	}			
			// }
			...
			if(f_img.length > 0){
				if(f_img[0].offsetHeight == 0){
					...
					f_img[0].style.setProperty("max-height", "900px", "important");
				}							
			}		
	...
	for(var f=0;f<figuretags.length;f++){			
		...
		// debugger;
			if(fig_next_elem_sib.nodeName=='FIGURE'){
				...
				// hor_fig_div.style.setProperty('width','100%','important');
				// hor_fig_div.style.setProperty('display','table','important');
				// hor_fig_div.style.setProperty('border-spacing','5px','important');				
		...
		var figure_imgs=figuretags[f].getElementsByTagName('img');
		for (var fg = 0; fg < figure_imgs.length; fg++) {
			if(!figure_imgs[fg].closest('figcaption') && !figure_imgs[fg].closest('table')){
				figure_imgs[fg].classList.add('mhhe-pod--img-fig');
			}			
		}		
		...
		if (figcaption[0]) {
			...
			if(figcaption[0].querySelectorAll("[class*=number]").length > 0){
				...
				if ((numbered_class_html[0].indexOf('fig.') > -1 || numbered_class_html[0].indexOf('figure') > -1 || numbered_class_html[0].indexOf('exhibit') > -1 || numbered_class_html[0].indexOf('module') > -1 || numbered_class_html[0].indexOf('chart') > -1) && !isNaN(cap_num) && (figuretags[f].querySelectorAll(':scope > img').length > 0 || figuretags[f].querySelectorAll(':scope > div > img').length > 0 || figuretags[f].querySelectorAll(':scope > div > div > img').length > 0 || figuretags[f].querySelectorAll(':scope > div > div > div > img').length > 0 || figuretags[f].querySelectorAll(':scope > figcaption > img').length > 0)) {

					if(figuretags[f].querySelectorAll(':scope > figcaption > img').length > 0){
						figuretags[f].querySelector(':scope > figcaption > img').classList.add('mhhe-pod--img-fig'); 
					}
					...
					if(!figuretags[f].classList.contains('mhhe-pod--image-75to50-wrap')){
						figuretags[f].classList.add('mhhe-pod--large-cap-fig');					
					}
					
					if(!figuretags[f].closest('table,aside,li,[role=region]') && !figuretags[f].classList.contains('mhhe-pod--image-75to50-wrap')){						
		...
		if(clos_li){
			if(figure.closest('ul')){
				if(figure.closest('ul').getElementsByTagName('li').length == 1){
					if(figure.closest('ul').getElementsByTagName('li')[0].children.length == 1){
				...
				if(figure.closest('ol').getElementsByTagName('li').length == 1){
					if(figure.closest('ol').getElementsByTagName('li')[0].children.length == 1){
...
figureChangesClass.prototype.calculateParaHeight= function(elem,elem_height,para,para_height,final_height,place="",fbreak=false,modeltype=""){
	...
	if(para.nodeName=='P' || para.nodeName=='SPAN' || para.nodeName=='UL' || para.nodeName=='OL'){		
		...
		if(para.closest(".mhhe-pod--glos-entry")){
			// 
			// var nxt_p_glos=para.closest(".mhhe-pod--glos-entry").nextElementSibling;
			// if(nxt_p_glos){
			// 	var total_p_height=(nxt_p_glos.offsetHeight)+para_height_percentage;
			// 	return this.calculateParaHeight(elem,elem_height,nxt_p_glos,total_p_height,final_height,updated_place,'',false,modeltype);
			// }
			return final_height;
		...
		if(elem.parentElement!=para.parentElement){		  				
	 	    ...
	 	    if(para.parentElement.previousElementSibling){
	 	    	...
	 	    	if(place==''){
					...
					updated_place=place;
					console.log(updated_place,para);
					elem.parentElement.insertBefore(para,place);
				}
	 	    }
		}
...
figureChangesClass.prototype.processNextElementWrapping= function(next_childrens,styles,elem,para,para_height_percentage,elem_height,elem_place,final_height,u_place="",mtype=""){
	if(elem_place){
		if(elem_place.closest('blockquote')){
	...
	for (var c = 0; c < next_childrens.length; c++) {
		...
		if(next_childrens[c].nodeName == 'HEADER'){
				...
				if(headerchildrens[nhc].nodeName=='H1'){
		 	    ...
		 	    for (var si=0; si < styles.length; si++) {
		 	    	if(styles[si]!='float' && styles[si]!='width' && styles[si]!='height'){
		 	    		header_styles += styles[si]+':'+this._projectlist.getPropValue(headerchildrens[nhc], styles[si])+';';
		 	    	}		 	       
		 	    }
		 	    if(headerchildrens[nhc].innerText.length >=33 && mtype=='fig'){
		 	    	header_styles+='margin-bottom:25px!important;display:inline!important;page-break-after:avoid !important;';
		 	    }
			...
			// if(nxt_p.id=='data-uuid-c3f18f8deaa2467089136a0e340f157e'){
			// 	debugger;
			// }
...
figureChangesClass.prototype.fixLargeFigCaption= function(){//mhhe-pod--image-75to50-wrap
	...
	for (var fc = 0; fc < figcaption.length; fc++) {
		...
		if(figcap_height > 20 && caption_parent.className.indexOf('mhhe-pod--image-75to50-wrap') <=-1 && parentfloat=='none' && valid_caption && caption_parent.className.indexOf('mhhe-pod--large-cap-fig') <=-1){
			...
			if(!caption_parent.classList.contains('mhhe-pod--image-75to50-wrap')){
...
figureChangesClass.prototype.setImageWidth= function(){
	var allTags = document.getElementById('hiddenfinalhtml').contentDocument;
	var figuretags=allTags.querySelectorAll("figure");
	for(var tw=0;tw<figuretags.length;tw++){
		
		var imgElem = figuretags[tw].getElementsByTagName("IMG");
		
		if(imgElem[0] && imgElem[0].parentNode){
			console.log("fig",imgElem[0],figuretags[tw].offsetHeight,figuretags[tw].offsetWidth)
			console.log("img",imgElem[0],imgElem[0].offsetHeight,imgElem[0].offsetWidth)
			if(figuretags[tw].classList.contains('mhe-twentyfive-container') && !figuretags[tw].classList.contains("one-half") && figuretags[tw].previousElementSibling != null){
			if(figuretags[tw].classList.contains("mhhe-pod--large-cap-fig")){
				imgElem[0].parentNode.style.setProperty('width','25%','important');
				imgElem[0].style.setProperty('width','50%','important');
			} 
			else {
				if(figuretags[tw].parentNode.parentNode && figuretags[tw].parentNode.parentNode.classList.contains("speaking") && (figuretags[tw].offsetHeight < figuretags[tw].offsetWidth)){
					imgElem[0].parentNode.style.setProperty('width','300px','important');
					imgElem[0].style.setProperty('width','400px','important');
				} else {
					imgElem[0].parentNode.style.setProperty('width','180px','important');
					imgElem[0].style.setProperty('width','180px','important');
				}
				
			}
		} else if(figuretags[tw].classList.contains('mhe-fifty-container') && !figuretags[tw].classList.contains("one-half") && figuretags[tw].previousElementSibling != null) {
			if(figuretags[tw].classList.contains("mhhe-pod--large-cap-fig")){
				imgElem[0].parentNode.style.setProperty('width','50%','important');
				imgElem[0].style.setProperty('width','75%','important');
			} else {
				imgElem[0].style.setProperty('width','200px','important');
			}
				
		}
		if(figuretags[tw].classList.contains('mhe-seventyfive-container') && !figuretags[tw].classList.contains("one-half") && !figuretags[tw].classList.contains("mhhe-pod--large-cap-fig")){
			if(figuretags[tw].previousElementSibling && figuretags[tw].previousElementSibling.classList.contains('mhhe-chapter_opener') && (figuretags[tw].offsetHeight < figuretags[tw].offsetWidth)){
				imgElem[0].parentNode.style.setProperty('width','100%','important');
				imgElem[0].style.setProperty('width','100%','important');
		} else {
			   imgElem[0].parentNode.style.setProperty('width','75%','important');
			  
		}
		}
		// if(figuretags[tw].classList.contains('mhe-seventyfive-container')&& !figuretags[tw].classList.contains("one-half") && figuretags[tw].classList.contains("mhhe-pod--large-cap-fig")){
		// 	//console.log(imgElem[0],"2yesss")
		// 	imgElem[0].parentNode.style.setProperty('width','100%','important');
		// 	//imgElem[0].parentNode.classList.add("add_right_float");
		// }
