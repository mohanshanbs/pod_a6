...
export class ProcessPDFService {
                                ...
                                if(allelems[y]){
                                    ...
                                    if(content==''){
                                        if(allelems[y].nodeName){
                                            if(allelems[y].nodeName!='TABLE' && allelems[y].nodeName!='TR' && allelems[y].nodeName!='TD' && allelems[y].nodeName!='TH' && allelems[y].nodeName!='COL' && allelems[y].nodeName!='COLGROUP' && allelems[y].className.indexOf('wrapperpg')<= -1 && allelems[y].nodeName!='IMG' && allelems[y].nodeName!='LINK' && allelems[y].nodeName!='META' && !allelems[y].closest('math') && allelems[y].nodeName!='BR' && allelems[y].nodeName!='IFRAME' && allelems[y].nodeName!='SPAN' && allelems[y].nodeName!='EM' && allelems[y].nodeName!='STRONG'){
                                                allelems[y].remove();
                                                y--;
                                            }
                                        }
      ...
      for (let i = 0; i < tablelist.length; i++) {
       ...
       if (curwidth > checkwidth && thistable[0].innerHTML.indexOf('<img') < 0) {
        ...
        thistable[0].innerHTML = thistable[0].innerHTML.replace(new RegExp("<col ", "g"), "<col style='font-size: 8pt !important; padding: 3px !important;margin: 0 !important;text-indent: 0 !important;'");
          ...
          if(totalheight > 690 && totalheight <= 1035){
          ...
          //(thistable[0].parentNode.parentNode.parentNode.parentNode as HTMLElement).setAttribute("rotate-table","true");
 ...
 setTableWidth(tableChildren, tmpwidth, type) {
    ...
    extractFile(filename, destination, project, url, userid, fextension) {
